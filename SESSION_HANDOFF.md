...
### 1) `npm run sync:notion` 첫 실행 — **사용자만 가능** (2-D 참고)
토큰이 필요합니다. `--dry`로 먼저 스키마를 확인하세요.

### 2) 노션 About 연동 — **차단됨** (4-2 참고)
`personInfo`는 여전히 동기화 대상이 아닙니다. 하려면 sync 스크립트에 About 페이지 경로를 추가해야 합니다.
